/* blg search - MiniSearch 7.1.1 (MIT) inlined + search client */

/* --- MiniSearch 7.1.1 (MIT License) --- */
/**
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // On the search page, prevent form submit and search live instead
  ...
  fetch('search-index.json')
    ...
    .then(function (data) {
      ...
      index = new MiniSearch({
        ...
        storeFields: ['title', 'url', 'body', 'date', 'tags'],
        searchOptions: {
  ...
  function doSearch(query) {
    ...
    for (var i = 0; i < hits.length; i++) {
      var hit = hits[i];
      html += '<li><article>'
        + '<a href="' + escHtml(hit.url) + '" class="post-link">'
        + '<div class="preview">'
        + '<h1>' + escHtml(hit.title) + '</h1>'
        + '<p>' + escHtml(truncate(hit.body, 200)) + '</p>'
        + '</div>'
        + '</a>'
        + '</article></li>';
    }
